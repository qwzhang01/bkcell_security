var selects = $("select.roundselect").toArray();
...
function rSelects() {
	for (i = 0; i < selects.length; i++) {
		// 只有设置了class="roundselect"的select才做特殊处理
...
function rOptions(i, name) {
	...
	for (n = 0; n < selects[i].options.length; n++) {
		...
		if (option_selected) {
			...
			_$('select_info_' + name).appendChild(
...
function mouseSelects(name) {
	...
	if (isIE) {
		...
		_$(sincn).onclick = new Function("clickSelects('" + name + "');");
		_$('select_info_' + name)
				.addEventListener("click", stopBubbling, false);
	}
...
function clickOptions(i, n, name) {
	...
	select_info.className = 'tag_select';
	select_info.style.cursor = 'pointer';
	_$('options_' + name).parentNode.insertBefore(select_info, _$('options_'
			+ name));
